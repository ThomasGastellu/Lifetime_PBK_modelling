...
##############################################################
### Smith et al. (2014)
Model_4<- (3e-04)*Body[["M"]][["BW"]]

Model_List_M <-cbind.data.frame(Model_4)
Body[["M"]]$Diaphragm <- Model_List_M

##############################################################
# 2b. female-specific physiological equations for models
##############################################################
Model_4 <- (3e-04)*Body[["F"]][["BW"]]

Model_List_F <-cbind.data.frame(Model_4)
Body[["F"]]$Diaphragm <- Model_List_F
...
Body_M <- Body[["M"]]
Body_M[12] <- unlist(Body[["M"]][["Diaphragm"]][["Model_4"]])
Body_M[["Gender"]]  <- "Male"

combined_models_M <- rbind(
  data.frame(Body_M, Model = "Model_4", "Volume (in L)" = Body_M[[12]]))

Body_F <- Body[["F"]]
Body_F[12] <- unlist(Body[["F"]][["Diaphragm"]][["Model_4"]])
Body_F[["Gender"]]  <- "Female"

combined_models_F <- rbind(
  data.frame(Body_F, Model = "Model_4", "Volume (in L)" = Body_F[[12]]))
...
LifeTimeChangesOrgans[["M"]]$Diaphragm <- Body[["M"]]$Diaphragm
LifeTimeChangesOrgans[["F"]]$Diaphragm <- Body[["F"]]$Diaphragm
