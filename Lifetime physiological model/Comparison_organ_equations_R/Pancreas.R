...
Model_1 <- 0.00192 * Body[["M"]][["BW"]] 

### Smith et al. (2014)
Model_4<- (1.480e-03)*Body[["M"]][["BW"]] 

### Ring et al. (2017)
Model_5<- (7.46*Body[["M"]][["HT"]] /100 * sqrt(Body[["M"]][["BW"]] ) - 0.79)/1.045/1000

Model_List_M <-cbind.data.frame(Model_1, Model_4, Model_5)
Body[["M"]]$Pancreas<- Model_List_M
...
### Beaudouin et al. (2010)
Model_1 <- 0.002 * Body[["F"]][["BW"]]

### Smith et al. (2014)
Model_4 <- (1.480e-03)*Body[["F"]][["BW"]]

### Ring et al. (2017)
Model_5 <- (7.92*Body[["F"]][["HT"]]/100 * sqrt(Body[["F"]][["BW"]]) - 2.09)/1.045/1000

Model_List_F <-cbind.data.frame(Model_1, Model_4, Model_5)
Body[["F"]]$Pancreas<- Model_List_F
...
Body_M <- Body[["M"]]
Body_M[12] <- unlist(Body[["M"]][["Pancreas"]][["Model_1"]])
Body_M[13] <- unlist(Body[["M"]][["Pancreas"]][["Model_4"]])
Body_M[14] <- unlist(Body[["M"]][["Pancreas"]][["Model_5"]])
Body_M[["Gender"]]  <- "Male"

combined_models_M <- rbind(
  data.frame(Body_M, Model = "Model_1", "Volume (in L)" = Body_M[[12]]),
  data.frame(Body_M, Model = "Model_4", "Volume (in L)" = Body_M[[13]]),
  data.frame(Body_M, Model = "Model_5", "Volume (in L)" = Body_M[[14]]))


Body_F <- Body[["F"]]
Body_F[12] <- unlist(Body[["F"]][["Pancreas"]][["Model_1"]])
Body_F[13] <- unlist(Body[["F"]][["Pancreas"]][["Model_4"]])
Body_F[14] <- unlist(Body[["F"]][["Pancreas"]][["Model_5"]])
Body_F[["Gender"]]  <- "Female"

combined_models_F <- rbind(
  data.frame(Body_F, Model = "Model_1", "Volume (in L)" = Body_F[[12]]),
  data.frame(Body_F, Model = "Model_4", "Volume (in L)" = Body_F[[13]]),
  data.frame(Body_F, Model = "Model_5", "Volume (in L)" = Body_F[[14]]))
...
LifeTimeChangesOrgans[["M"]]$Pancreas <- Body[["M"]]$Pancreas
LifeTimeChangesOrgans[["F"]]$Pancreas <- Body[["F"]]$Pancreas
